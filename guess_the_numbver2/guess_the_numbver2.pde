...
void draw() {
  ...
  textSize(16);
  fill(0);
  text("Guessed Numbers: ", 300, 550);
  for (int i = 0; i < numGuesses; i++) {
    text(guessednumbers[i] + " ", 440 + i * 30, 550);
  }
  ...
  // "Try again" message for 1 seconds
  if (showTryAgainMessage && millis() - tryAgainMessageStartTime < 1000) {
    textSize(24);
...
void TextInput1(String input) {
  ...
  guessednumbers[numGuesses] = guessedNumber;
  numGuesses++;
